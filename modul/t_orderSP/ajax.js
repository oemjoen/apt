...
//	$("#txt_tgl_beli").datepicker("setDate", Now.Date);
	...
	function cari_nomor() {
		...
		$.ajax({
			type	: "POST",
			url		: "modul/t_orderSP/cari_nomor.php",
	...
	function tampil_data() {
		...
		$.ajax({
				type	: "POST",
				url		: "modul/t_orderSP/tampil_data.php",
				data	: "kode="+kode,
				//timeout	: 6000,
				beforeSend	: function(){		
	...
	$("#txt_kode_barang").autocomplete("modul/t_orderSP/list_barang.php", {
				width:100,
				//max: 10,
				scroll:true,
	});
	...
	function cari_kode() {
		...
		$.ajax({
			type	: "POST",
			url		: "modul/t_orderSP/cari_barang.php",
	...
	$("#cbo_retail").change(function() {
		...
		$.ajax({
			type	: "POST",
			url		: "modul/t_orderSP/tampil_limit.php",
	...
	$("#proses").click(function() {
	   ...
	   var pilih = confirm('Data yang akan diproses kode = '+kode+ '?');
		if (pilih==true) {
			$.ajax({
				type	: "POST",
				url 	: "modul/t_orderSP/proses.php",
				data	: "kode="+kode,
				success	: function(data){
					$("#info").html(data);
                    kosong();
...
//		var tipe			= $("#cbox1").val();
	...
	if(jumlah.length == 0){
		...
		if(error == false){
			$.ajax({
				type	: "POST",
				url		: "modul/t_orderSP/simpan.php",
				data	: "kode="+kode+
							"&tgl="+tgl+
							"&kodeO="+kodeO+
