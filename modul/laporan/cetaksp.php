...
//		  SELECT * FROM t_order WHERE NoOrder='ORD/BDGAPCICAH/00001'
			$query 	= "SELECT a.`Cabang`, a.`KodeApotik`,b.`Nama Faktur` AS nmApt, b.`Alamat` AS almApt,
                        	d.`Nama Faktur` AS nmF,a.`KodeProduk`,c.`Produk`,c.`Satuan`,
                        	a.`NoOrder`,SUM(a.Banyak) AS Banyak, ROUND(AVG(a.Diskon),2) AS Diskon, SUM(a.Total) AS Total,a.TglSP
                        FROM t_order a, mpelanggan b, mproduk c, mretail d
                        WHERE a.NoSP='$kode' AND
                        	a.`KodeProduk`=c.`Kode Produk` AND a.`KodeApotik`=b.`Kode` AND
                        	a.`KodePelanggan`=d.`Kode`
                        GROUP BY a.NoSP,a.KodeProduk";
			}	 
...
/**
		 ...
		 */		}else{
			$query 	= "SELECT a.`Cabang`, a.`KodeApotik`,b.`Nama Faktur`,
                        	d.`Nama Faktur` AS nmF,a.`KodeProduk`,c.`Produk`,c.`Satuan`,
                        	a.`NoOrder`,SUM(a.Banyak) AS Banyak,SUM(a.Qty) AS Qty,SUM(a.Bonus) AS Bonus, 
                            ROUND(AVG(a.Diskon),2) AS Diskon, SUM(a.Total) AS Total,a.TglSP
                        FROM t_order a, mpelanggan b, mproduk c, mretail d
                        WHERE a.NoSP='$kode' AND
                        	a.`KodeProduk`=c.`Kode Produk` AND a.`KodeApotik`=b.`Kode` AND
                        	a.`KodePelanggan`=d.`Kode`
		...
		$w=array(5,15,15,105,20,25,15,10);
...
//		  $this->Cell($w[0],6,$no,1,0,'C',$fill);
		  $this->Cell($w[1],6,$data['Qty'],1,0,'C',$fill);
		  $this->Cell($w[7],6,$data['Bonus'],1,0,'C',$fill);
...
//    $pdf->Cell(0,5,$apotik,0,1,'L');
    ...
    $pdf->Cell(105,5,'No. SP           : '.$kode,0,0,'L');
    $pdf->Cell(105,5,'Kepada Yth: ',0,1,'R');
    $pdf->Cell(105,5,'Tanggal          : '.tgl_indo($tgl),0,0,'L');
	$pdf->Cell(105,5,"PT.SAPTASARITAMA",0,1,'R');
	$pdf->Cell(210,5,"Cabang ".$cbg,0,1,'R');
    $pdf->SetX(10);
	$pdf->Ln(1);
...
//	$pdf->Cell(5,5,'No.',1,0,'C',true);
	...
	$pdf->Cell(10,5,'Bonus',1,0,'C',true);
	...
	$pdf->Cell(25,5,'Value',1,1,'C',true);
	...
	$pdf->Output('Order_'.$apotik.'_'.$kode.'.pdf','D'); 
/**
