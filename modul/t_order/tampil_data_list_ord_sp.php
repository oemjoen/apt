<script type="text/javascript">
	...
	function cetakRow(ID){
	   var kode = ID; 
		window.location.href='modul/laporan/cetaksp.php?kode='+kode;
...
if (empty($kode))
{
    if ($cabang == 'KPS')
    {
        $query2 = "SELECT a.NoSP,a.`TglSP`,a.`KodeApotik`,c.`Nama Faktur` AS NmFak,a.`Status`,SUM(a.`Value`) AS tot1 FROM t_order a, mpelanggan c, mretail d
						WHERE a.`KodeApotik`=c.`Kode` AND a.`KodePelanggan`=d.`Kode` AND a.`Status` in ('OpenSP','ProsesSP')
...
} else
{
    if ($cabang == 'KPS')
    {
        $query2 = "SELECT a.`NoSP`,a.`TglSP`,a.`KodeApotik`,c.`Nama Faktur` AS NmFak,a.`Status`,SUM(a.`Value`) AS tot1 FROM t_order a, mpelanggan c, mretail d
							WHERE a.`KodeApotik`=c.`Kode` AND a.`KodePelanggan`=d.`Kode` AND a.`Status` in ('OpenSP','ProsesSP')
							AND a.KodeApotik='$apt' AND NoSP = '$kode'"; //echo "--3--".$query2;
    } else
    {
        $query2 = "SELECT a.`NoSP`,a.`TglSP`,a.`KodeApotik`,c.`Nama Faktur` AS NmFak,a.`Status`,SUM(a.`Value`) AS tot1 FROM t_order a, mpelanggan c, mretail d
							WHERE a.`KodeApotik`=c.`Kode` AND a.`KodePelanggan`=d.`Kode` AND a.`Status` in ('OpenSP','ProsesSP')
							AND NoSP = '$kode' AND cabang='$cabang'"; //echo "--4--".$query2;
    }
}
...
?> 
                    onClick="$.get('modul/t_order/tampil_data_list_ord_sp.php?cabang=<?php
