<script type="text/javascript">
	...
	function AppAptRow(ID){
		...
		var namauser	= $("#textnamauser").val();
	   	...
	   	$.ajax({
			type	: "GET",
			url		: "modul/lunas/tampil_data_list_po_apt.php",
...
if (empty($kode)){
	if ($cabang =='KPS')
	{
		$query2 = "SELECT DISTINCT a.`kodepo_beli`,a.`tglpo_beli`,a.`kode_beli`,a.`kode_prinsipal`,d.`namaprinsipal`,a.`kode_supplier`,b.`namasupplier` 
					FROM po_pembelian a
						...
						LEFT JOIN `mstprinsipal` d ON a.`kode_prinsipal`=d.`kodeprinsipal` WHERE a.tglpo_beli >= DATE(NOW() - INTERVAL 3 MONTH)
	...
	else
	{
		$query2 = "SELECT DISTINCT a.`kodepo_beli`,a.`tglpo_beli`,a.`kode_beli`,a.`kode_prinsipal`,d.`namaprinsipal`,a.`kode_supplier`,b.`namasupplier` 
					...
					WHERE a.cabang='$cabang' and a.tglpo_beli >= DATE(NOW() - INTERVAL 3 MONTH)
...
-->				<th>CRutin</th>				
		...
		if (empty($kode)){
			if ($cabang =='KPS'){
					$sql = "SELECT DISTINCT a.`kodepo_beli`,a.`tglpo_beli`,a.`kode_beli`,a.`kode_prinsipal`,d.`namaprinsipal`,a.`kode_supplier`,b.`namasupplier` ,status_apv_apt,status_batal
							FROM po_pembelian a
								LEFT JOIN mstsupplier2 b ON a.`kode_supplier`=b.`kodesupplier`
								LEFT JOIN `mstprinsipal` d ON a.`kode_prinsipal`=d.`kodeprinsipal` WHERE a.tglpo_beli >= DATE(NOW() - INTERVAL 3 MONTH)
		...
		else
			{
				if ($cabang =='KPS'){
						$sql = "SELECT a.`kodepo_beli`,a.`tglpo_beli`,a.`kode_supplier`,b.`namasupplier`,a.`kode_barang`,c.`namaproduk`,c.`satuan`,
							a.`jumlah_beli_valid`,status_apv_apt,status_batal
							FROM `po_pembelian` a 
							LEFT JOIN `mstsupplier2` b ON a.`kode_supplier`=b.`kodesupplier`
