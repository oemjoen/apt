...
$(document).ready(function() {
	$("#data").load('modul/po_pembelian/tampil_data_list_po.php?cabang='+$("#textcabang").val()+'&user='+$("#textnamauser").val());
...
/*		if (kode1.length==''){
		...
		if (kode2.length==''){
	   	...
	   	$.ajax({
			type	: "GET",
			url		: "modul/po_pembelian/tampil_data_list_po.php",
...
#info {
	...
	<p align="right">
	  ...
	  <input type="hidden" name="textnamauser" id='textnamauser' value = "<?php echo $userrr; ?>"/>
      USER : <?php echo $namarr;?> - CABANG : <?php echo $cabangrr; ?></p>

<div align='center' id='form'>
  <h2><strong>LIST / CETAK PO RUTIN</strong></h2>
</div>
	...
	<fieldset>
		...
		<table width='100%'>
			<tr>
				...
				<?php 
				if ($cabangrr == 'KPS') {
				$query	= "SELECT DISTINCT(kodepo_beli) AS kodepo_beli,`tglpo_beli`,cabang FROM po_pembelian HAVING (tglpo_beli - INTERVAL  '1' MONTH )
